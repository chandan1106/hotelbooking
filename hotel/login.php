<?php
/**
 * Hotel Owner Login Page
 */
$pageTitle = 'Hotel Owner Login';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        ...
        $db->query("SELECT * FROM hotel_owners WHERE email = :email");
        $db->bind(':email', $email);
        $hotelOwner = $db->single();
        
        if ($hotelOwner && password_verify($password, $hotelOwner['password'])) {
            // Check subscription status
            $subscriptionStatus = getSubscriptionStatus($hotelOwner['id'], $db);
            
            // Set session variables
            $_SESSION['hotel_owner_id'] = $hotelOwner['id'];
            $_SESSION['hotel_owner_name'] = $hotelOwner['first_name'] . ' ' . $hotelOwner['last_name'];
            $_SESSION['hotel_owner_email'] = $hotelOwner['email'];
            $_SESSION['subscription_status'] = $subscriptionStatus;
            
            // Update last login time
            $db->query("UPDATE hotel_owners SET last_login = NOW() WHERE id = :id");
            $db->bind(':id', $hotelOwner['id']);
            $db->execute();
            ...
            if ($rememberMe) {
                ...
                $db->query("UPDATE hotel_owners SET remember_token = :token, remember_expiry = :expiry WHERE id = :id");
                $db->bind(':token', $token);
                ...
                // Set cookie
                setcookie('remember_hotel_owner', $hotelOwner['id'] . ':' . $token, $expiry, '/');
            }
            
            // Redirect based on subscription status
            if ($subscriptionStatus == 'expired') {
                $_SESSION['error_message'] = 'Your subscription has expired. Please renew to continue using our services.';
...
<div class="max-w-md mx-auto bg-white rounded-lg shadow-md p-6 mt-8">
    <h2 class="text-2xl font-bold mb-6 text-center">Hotel Owner Login</h2>
    ...
    <div class="mt-6 text-center">
        <p class="text-gray-600">Don't have an account? <a href="register.php" class="text-blue-600 hover:underline">Register as Hotel Owner</a></p>
    </div>
</div>
